...
Never include a real API key, `.env` contents, authorization headers or private game files in a report. Revoke an exposed key through its provider before sharing a sanitized reproduction.
...
The Python controller sends structured game state and candidate actions to TypeSafe over HTTPS. It reads `TYPESAFE_API_KEY` (or the older `JEV_KEY`) from the environment or the explicitly selected `.env` file. The supplied launcher removes both variables from the game process environment. The mod does not need the API credential.
