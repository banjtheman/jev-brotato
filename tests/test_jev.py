...
class JevClientTests(unittest.TestCase):
    def test_official_request_contract_and_metadata(self):
        ...
        with patch("brotato_agent.jev.time.perf_counter", side_effect=[1.0, 1.25]):
    ...
    def test_default_transport_reuses_connection_without_replaying_failure(self):
        from brotato_agent.jev import _PersistentHTTPS
        from urllib.request import Request
        request = Request(ENDPOINT, data=b"{}", method="POST")
        with patch("brotato_agent.jev.HTTPSConnection") as factory:
    ...
    def test_retries_only_rate_limit_or_overload_and_are_bounded(self):
        for status in (429, 529):
            ...
            with patch("brotato_agent.jev.time.sleep") as sleep:
        ...
        with patch("brotato_agent.jev.time.sleep"), self.assertRaises(JevAPIError):
    ...
    def test_retry_after_too_long_does_not_retry_early(self):
        ...
        with patch("brotato_agent.jev.time.sleep") as sleep, self.assertRaises(JevAPIError):
